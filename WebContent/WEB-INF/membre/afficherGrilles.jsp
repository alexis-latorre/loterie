	...
	<c:choose>
		<c:when test="${not empty grilles}">
			<c:forEach items="${grilles}" var="grille">
				...
				<div class="well">
					<a class="titre-grille main-text-blue" title="Accéder aux détails de cette grille" href="<c:url value="/membre/afficherGrille"><c:param name="id" value="${grille.id}" /></c:url>">${grille.nom}</a>
					<br />
					<!-- <c:choose>
			...
			<p><a class="main-text-gold" href="<c:url value="/membre/creerGrille" />">Ajouter une grille</a></p>
		</c:when>
		<c:otherwise>
			<p class="main-text-gold">Vous ne participez à aucune grille.<br />
